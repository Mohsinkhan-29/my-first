...
const Navbar = ({ handleOrderPopup }) => {
  return (
    <div className="shadow-md bg-white dark:bg-gray-900 dark:text-white duration-200 relative z-40 ">
      {/* upper Navbar */}
      <div className='bg-primary/40 py-2 px-10'>
        <div className='container flex justify-between items-center'>
          <div>
            <a href="#" className='font-bold text-bold text-2xl sm:text-3xl flex gap-2'>
          ...
          <div className='flex justify-between items-center gap-4'>
            ...
            <button onClick={() => handleOrderPopup()}
              ...
              <TbShoppingCartBolt className='text-xl text-white drop-shadow-sm cursor-pointer' />
            </button>
        ...
        <ul className='sm:flex hidden items-center'>{
          ...
          <li className='group  relative cursor-pointer'>
            <a href="#" className='flex items-center gap-[2px] py-2'>Trending Products
              <span>
                <PiCaretCircleDoubleDownFill className="transition-all duration-200 group-hover:rotate-180" />
              </span>
            </a>
            <div className='absolute z-[9999] hidden group-hover:block w-[150px] rounded-md bg-white p-2 text-black shadow-md' >
              <ul>
                {Dropdownlinks.map((data) => (
                  <li>
                    <a href={data.id} className='inline-block w-full rounded-md p-2 hover:bg-primary/20'>{data.name}</a>
                  </li>
                ))}
              </ul>
            </div>
